    ...
    override open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if (indexPath as NSIndexPath).row >= prefixDays {
            ...
            if arrSelectedDates.filter({ $0.isDateSameDay(currentDate)
                ...
                if nextMonthFirstDay.compare(currentDate) == .orderedAscending {
                ...
                if startDate != nil {
                    let start1 = Calendar.current.startOfDay(for: cell.currentDate as Date)
                    let start2 = Calendar.current.startOfDay(for: startDate!)
                    if  start1.compare(start2) == .orderedAscending {
                        cell.isCellSelectable = false
                        cell.lblDay.textColor = self.dayDisabledTintColor
                    }
                }
